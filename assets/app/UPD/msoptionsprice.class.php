...
class msoptionsprice
{
    ...
    public function getOldCostByModification($modification = array(), $isAjax = false)
    {
        ...
        if (is_null($cost)) {
      	...
      	switch ($product->get('currency')) {
			    case 1:
			        $oldPrice = $this->modx->getOption('old_price', $modification) * $this->modx->getOption('RUB') / 100;
			        break;
			    case 2:
			        $oldPrice = $this->modx->getOption('old_price', $modification) * $this->modx->getOption('EUR');
			        break;
			    default:
    ...
    public function getCostByModification($rid = 0, $price = 0, $modification = array(), $isAjax = false)
    {
        ...
        $product_cur = $this->modx->getObject('msProduct', array('id' => (int)$rid));
        
        switch ($product_cur->get('currency')) {
				    case 1:
				        $cost = $this->modx->getOption('price', $modification, 0, true) * $this->modx->getOption('RUB') / 100;
				        $price = $price * $this->modx->getOption('RUB') / 100;
				        break;
				    case 2:
				        $cost = $this->modx->getOption('price', $modification, 0, true) * $this->modx->getOption('EUR');
				        $price = $price * $this->modx->getOption('EUR');
				        break;
				    default:
