msOrderFiles.grid.Default = function (config) {
    config = config || {};

    if (typeof(config['multi_select']) != 'undefined' && config['multi_select'] == true) {
        config['sm'] = new Ext.grid.CheckboxSelectionModel();
    }
    config['cls'] = (config['cls'] || 'main-wrapper') + ' msof-grid';
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('msof-grid__row_disabled');
                }
                return cls.join(' ');
            },
        },
    ...
    msOrderFiles.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(msOrderFiles.grid.Default, MODx.grid.Grid, {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = msOrderFiles.utils.getMenu(row.data['actions'], this, ids);
    ...
    _doAction: function (action, action_prefix, confirm, check_id) {
        ...
        if (confirm) {
            MODx.msg.confirm(Ext.apply({}, params, {
                title: ids.length > 1
                    ? _('msorderfiles_button_' + action + '_multiple')
                    : _('msorderfiles_button_' + action),
                text: _('msorderfiles_confirm_' + action),
            }));
        } else {
...
Ext.reg('msof-grid-default', msOrderFiles.grid.Default);