...
class msDiscount {
	...
	public function getNewPrice($product_id, $price, $user_id = '', $date = '') {
		...
		if ($this->percent != '0%' && $this->modx->getOption('msd_price_round', null, false)) {
			$price = round($price);
		}
	...
	/**
	 * Compare discount witsh current discounts
	 *
	 * @param $discount
	 ...
	 * @return array
	 */
	public function compareDiscount($discount, $discounts = array(), $ends = '0000-00-00 00:00:00') {
		if (empty($discounts)) {
			$discounts = array(
		...
		if (strpos($discount, '%') !== false) {
			if ((float) ($discount) > (float) $discounts['percent']) {
				$discounts['percent'] = $discount;
				$discounts['remains'] = ($ends != '0000-00-00 00:00:00')
					? strtotime($ends) - time()
					: 0;
			}
		}
		elseif ((float) $discount > (float) $discounts['absolute']) {
			$discounts['absolute'] = $discount;
			$discounts['remains'] = ($ends != '0000-00-00 00:00:00')
				? strtotime($ends) - time()
				: 0;
		}
	...
	public function getCouponDiscount($code, $price) {
		...
		if ($this->modx->getOption('msd_price_round', null, false)) {
			$res = round($res);
		}
