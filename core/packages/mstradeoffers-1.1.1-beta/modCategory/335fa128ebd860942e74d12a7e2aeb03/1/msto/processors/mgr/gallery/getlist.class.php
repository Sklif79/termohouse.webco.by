...
class mstoProductFileGetListProcessor extends modObjectGetListProcessor
{
    ...
    public function prepareQueryBeforeCount(xPDOQuery $c)
    {
		...
		/* $offer_id = $this->getProperty('offer_id');
		if($offer_id != 0){
			$c->leftJoin('mstoOfferFile', 'File', 'msProductFile.id = File.image_id');
			 $c->select('File.id as checked');
		} */

		 $color = $this->getProperty('color');
		 if(!empty($color)){
			$c->leftJoin('mstoOfferFile', 'File', 'msProductFile.id = File.image_id AND File.color = "'.$color.'"');
			$c->select('File.id as checked');
		}
    ...
    public function prepareArray(array $row)
    {
		...
		if($row['checked']) $row['checked'] = 'checked';
        $row['actions'] = array();
...
return 'mstoProductFileGetListProcessor';